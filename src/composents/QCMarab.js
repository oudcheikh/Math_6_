...
function QCM({ questionsArray }) {
  ...
  const handleFlip = (questionIndex) => {
    if (isFlipped === questionIndex) {
      ...
      setCorrect((prev) => ({
        ...prev,
        [questionIndex]: selectedAnswers[questionIndex] === correctAnswer,
      }));
    }
  };
          ...
          <Card className={isFlipped === index ? "flipped" : "not-flipped"}>
            {isFlipped !== index ? (
              <CardContent>
                ...
                <RadioGroup value={selectedAnswers[index] || ""} onChange={(e) => handleAnswerChange(index, e.target.value)}>
                  {question.answers.map((answer, aIndex) => (
                    <FormControlLabel key={aIndex} value={answer} control={<Radio />} label={<StyledText>{answer}</StyledText>} />
                  ))}
                </RadioGroup>

                <Button variant="contained" color="primary" style={{ padding: "-18px 10px" }} onClick={() => handleFlip(index)}>
                  شرح
                </Button>
              </CardContent>
            ) : (
              <CardContent>
                <Typography variant="h5" color={correct[index] ? "green" : "red"}>
                  <StyledText>{correct[index] ? "إجابة صحيحة!" : "إجابة خاطئة"}</StyledText>
                </Typography>

                <Typography variant="h6">
                  <StyledText>شرح:</StyledText>
                </Typography>

                <Typography variant="body1">
                  <StyledText>{question.explanation}</StyledText>
                </Typography>
